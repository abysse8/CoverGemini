...
def build_session_document_context(session: Dict[str, Any]) -> Dict[str, Any]:
    context_files = []

    for item in session.get("context_files", []):
        path = item.get("path", "")
...
def generate_letter(session: Dict[str, Any]) -> Dict[str, Any]:
    system = (
        ...
        "Utilise les documents de contexte fournis, notamment les dossiers de compétences, anciens CV et fichiers uploadés. "
        "N'invente rien qui ne soit pas soutenu par le profil ou les documents. "
        "Conclusion directe vers l'entretien."
    )
    document_context = build_session_document_context(session)
    contents = [{
        "role": "user",
        "parts": [{"text": json.dumps({
            "job_target": session["job_target"],
...
def ingest_file_route():
    ...
    file_path = os.path.join(ingest_dir, safe_name)
    file.save(file_path)

    try:
        text = extract_text_from_file(file_path, safe_name)

        session.setdefault("context_files", [])
        ...
        result = ingest_text_into_profile(session, text, safe_name)
        return jsonify({
            "session_id": session_id,
            "filename": safe_name,
            "profile": session["profile"],
...
@app.route("/session/context/<session_id>", methods=["GET"])
def session_context(session_id):
    ...
    return jsonify({
        ...
        "loaded_context": build_session_document_context(session),
    })
    ...
    try:
        ...
        for key in request.files:
            ...
            for file in file_list:
                ...
                log(f"Saved for compile job {job_id}: {safe_name}")
